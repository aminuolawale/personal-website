        ...
        {loading ? (
          ...
          <div className="divide-y divide-surface/[0.06]">
            {projectList.map((p) => (
              <div
                key={p.id}
                className="py-4 flex items-center justify-between gap-4 hover:bg-surface/[0.015] -mx-3 px-3 transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="font-mono text-[10px] text-accent/60 border border-accent/20 px-1.5 py-0.5">
                      pos {p.position}
                    </span>
                    <span className={`font-mono text-[10px] ${p.published ? "text-emerald-400/70" : "text-muted/30"}`}>
                      {p.published ? "Published" : "Draft"}
                    </span>
                  </div>
                  <p className="text-surface font-semibold truncate">{p.title}</p>
                  {p.description && (
                    <p className="font-mono text-xs text-muted/35 mt-0.5 truncate">{p.description}</p>
                  )}
                  <div className="flex items-center gap-3 mt-1">
                    {p.githubUrl && (
                      <a href={p.githubUrl} target="_blank" rel="noopener noreferrer" className="text-muted/30 hover:text-accent transition-colors">
                        <GitHubIcon size={13} />
                      </a>
                    )}
                    {p.websiteUrl && (
                      <a href={p.websiteUrl} target="_blank" rel="noopener noreferrer" className="text-muted/30 hover:text-accent transition-colors">
